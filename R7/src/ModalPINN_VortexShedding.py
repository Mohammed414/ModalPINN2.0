...
parser.add_argument('--PhaseCInit',type=float,default=0.85,help="Initial value of the single trainable convection-deficit scalar c in --PhaseLoss's target k_x = -c*omega_0/u0. NOT fixed/hardcoded - the network fits its own c jointly with the weights (a plain trainable tf.Variable, picked up automatically by declare_LBFGS/declare_Adam's default tf.trainable_variables() list), since hardcoding the ~0.891 measured against the true field in the audit would leak privileged full-field information into training. This is only the STARTING point for that fit.")
...
print('Phase C Init : %.4g' % (args.PhaseCInit))
...
# Trainable convection-deficit scalar c in k_x_target = -c*omega_0/u0. A
# plain trainable tf.Variable (trainable=True is the default) is picked up
# automatically by declare_LBFGS/declare_Adam's default tf.trainable_variables()
# list - same mechanism x_front_var's docstring documents for the OPPOSITE
# (trainable=False, externally-scheduled) case. Deliberately NOT hardcoded to
# the ~0.891 measured against the true CFD field in the audit - that number
# came from privileged full-field truth; the network fits its own value here,
# using only this term (self-referential: reads its own k=0 mode) plus the
# tap loss.
phase_c_var = tf.Variable(args.PhaseCInit, dtype=tf.float32, trainable=True, name='phase_c')
...
if args.PhaseLoss:
    ...
    print('Phase c final value : %.4f (audit measured 0.891 +/- 0.021 on the true field;'
          ' this is the network''s own fit, not the same number by construction)' % sess.run(phase_c_var))
