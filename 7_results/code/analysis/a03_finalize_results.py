...
Evaluation count is not an accuracy proxy: a longer run can have a lower loss
and a worse field. The comparison is therefore descriptive rather than a
causal estimate of the sampling effect. Its robust conclusion is only that no
tested arm recovers the downstream travelling wake.
"""
...
METHOD_NOTES = {
    "uniform_collocation": (
        ...
        "Observed endpoint after 5,503 L-BFGS evaluations; matched-effort re-run "
        "attempted and discarded 2026-08-28; not a one-sided accuracy bound."
    ),
...
def main() -> None:
    ...
    rows = []
    source = "6_analysis/derived/a03_collocation_summary.csv"
    ...
    for method, evals in LBFGS_EVALS.items():
        rows.append({
            ...
            "source": "6_analysis/derived/a03_input_manifest.json",
            "status": "accepted",
            "notes": (
                "Effort is not controlled across A03 arms. Evaluation count is "
                "reported for transparency but is not an accuracy proxy, so "
                "between-arm differences are not assigned causally to sampling."
            ),
        })
