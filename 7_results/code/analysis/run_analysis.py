...
def resolve_data_file() -> Path:
    ...
    candidates = (
        HERE.parents[2] / "GappyPOD" / "1_data" / "flow_cache.npz",
        HERE.parents[1] / "1_data" / "flow_cache.npz",
        HERE.parent / "1_data" / "flow_cache.npz",
    )
...
def temporal_modes(field: np.ndarray, times: np.ndarray, kmax: int = 3) -> np.ndarray:
    ...
    Fitting against shifted time keeps the least-squares system well
    conditioned; the final rotation expresses every mode against absolute
    time, which is the convention used by the ModalPINN coefficients.
    """
...
def main() -> None:
    ...
    configuration = {
        ...
        "data_file": "1_data/flow_cache.npz",
