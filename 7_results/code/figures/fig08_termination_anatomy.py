...
TRACES = np.load(ROOT / "data" / "census_traces.npz")
from figure_common import COLORS, new_figure, save_figure, check_text_overlaps  # noqa: E402



def trace(name):
    return TRACES[f'trace__{name}']


def nwarn(name):
    return int(TRACES[f'nwarn__{name}'])
...
runs = {name: dict(A=trace(name), nwarn=nwarn(name))
        for name in sorted(k[7:] for k in TRACES.files if k.startswith('trace__'))}
...
print(save_figure(fig, ROOT / "figures" / 'F_termination_anatomy'))
