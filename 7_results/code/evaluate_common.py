...
* the first-harmonic coefficient is ``0.5*(cos_coeff - 1j*sin_coeff)``
  in the absolute-time convention used by the ModalPINN forward model;
...
def temporal_harmonic_coefficients(
    ...
    ``(kmax + 1, node)`` and follows the absolute-time convention
    ``q(t)=q0 + q1 exp(i omega t) + conj(q1) exp(-i omega t) + ...``.

    The least-squares design is evaluated at ``tau=t-t0`` for numerical
    conditioning, then each fitted coefficient is rotated by
    ``exp(-i*k*omega*t0)``.  That final rotation is essential when a fitted
    CFD coefficient is compared directly with a saved ModalPINN mode, whose
    forward model uses absolute ``t``.  Omitting it leaves amplitude and
    magnitude-only correlation unchanged but biases relative error and signed
    phase by the fixed time-origin rotation.
    """
    ...
    for k in range(1, int(kmax) + 1):
        window_coefficient = 0.5 * (coefficients[2 * k - 1] -
                                    1j * coefficients[2 * k])
        modes[k] = window_coefficient * np.exp(-1j * k * omega * times[0])
    return modes
