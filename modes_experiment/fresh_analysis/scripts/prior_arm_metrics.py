...
1.  **First-harmonic phase convention.**  ``evaluate_common`` refers the true
    v1 to ``tau = t - t0`` (t0 = 400), while the network's modal coefficient is
    referred to absolute ``t``.  The two differ by a fixed rotation of
    ``omega_0*t0 = 343.37 deg``, so under the window convention a *perfect*
    model scores ``rel_L2 = 0.289`` and ``phase_deg = +16.63`` rather than 0.
    Here the truth is rotated into the network's convention, making ``rel_L2``
    and ``phase_deg`` mean what they claim to mean, and the legacy
    window-convention values are reported alongside (exactly, via
    ``arm_inputs.rotate_metrics``) so these numbers stay comparable with the
    already-published A01/A02/A04 rows until those are re-emitted.
...
def evaluate(*, analysis_id: str, method: str, arms: Mapping[str, pathlib.Path],
    ...
    v1_window = true_modes[1]
    theta_deg = phase_rotation_deg(float(times[0]), OMEGA_0)
    v1_absolute = v1_window * np.exp(-1j * OMEGA_0 * float(times[0]))
    ...
    output = {
        ...
        "v1_phase_convention": {
            ...
            "legacy": "window tau = t - t0 (as published for A01/A02/A04); a perfect model scores rel_L2 0.289 and phase +16.63 deg",
