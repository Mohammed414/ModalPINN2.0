...
def effort_audit(entries: Mapping[str, Mapping[str, object]]) -> Dict[str, object]:
    ...
    return {
        ...
        "note": (mechanism +
                 ...
                 "An arm with less effort that still performs well supports its claim "
                 "as a lower bound; an arm with less effort that performs badly is "
                 "uninformative."),
    }
...
def phase_rotation_deg(t0: float, omega: float) -> float:
    ...
    ``temporal_harmonic_coefficients`` refers the truth to ``tau = t - t0``
    (t0 = 400 for this dataset), while the network's modal coefficient is
    referred to absolute ``t`` (``NN_time_uv`` multiplies by
    ``exp(i k omega t)`` on the fed times).  The two differ by a constant
    rotation of ``omega*t0``: with the window-referenced truth, a perfect model
    reports ``phase_deg = +16.63`` and ``rel_L2 = 0.289`` rather than 0.
    """
