...
def main():
    ...
    street_params = None
    if args.StreetPrior is not None or 'TRUST' in run_dir_name:
        sp_path = args.StreetPrior
        if sp_path is None:
            cand = glob.glob(os.path.join(args.RunDir, 'street_prior*.npz'))
            assert cand, ("Run dir name contains 'TRUST' but no street_prior*.npz found in it and "
                          "--StreetPrior not given - cannot restore the trust ansatz.")
            sp_path = cand[0]
        _sp = np.load(sp_path)
        street_params = {k: float(_sp[k]) for k in
                         ('Gamma', 'Uc', 'xf', 'r0', 'omega', 'phase',
                          'amp_scale', 'scale_p', 'ramp', 'delta')}
        print('Restoring with TrustStreet prior from', sp_path)
