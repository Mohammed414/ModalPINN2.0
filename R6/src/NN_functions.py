...
def declare_LBFGS(loss,maxit=50000,maxfun=50000,ftol=1.0 * np.finfo(float).eps):
    # R6: 'disp': True makes scipy's own L-BFGS-B wrapper print its actual
    # termination message (e.g. CONVERGENCE vs ABNORMAL_TERMINATION_IN_LNSRCH)
    # via a plain Python print() - the R5 plan claimed this was "now logged"
    # but it never actually was (ScipyOptimizerInterface.minimize() doesn't
    # return or print the scipy OptimizeResult on its own). Needed to tell a
    # genuine fast convergence apart from a line-search failure - R5's own
    # early termination (4574 it vs R3's ~15452) was never actually diagnosed
    # either way.
