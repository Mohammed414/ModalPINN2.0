...
parser.add_argument('--ModalPressureLoss',action="store_true",default=False,help="R17: replace the time-domain cylinder pressure MSE with an equalized per-Fourier-mode relative pressure loss. The scalar weight is calibrated once at the common warm start so the initial total pressure contribution matches the ordinary time-domain pressure loss.")
parser.add_argument('--ModalPressureTargets',type=str,default=None,help="R17 .npz file containing the 32-tap least-squares pressure Fourier coefficients in the exact ModalPINN temporal convention.")
...
if args.ModalPressureLoss:
    if not args.PressureOnly:
        raise ValueError('--ModalPressureLoss requires --PressureOnly.')
    if args.ModalPressureTargets is None:
        raise ValueError('--ModalPressureLoss requires --ModalPressureTargets <npz>.')
...
print('R17 Modal Pressure Loss : ' + str(args.ModalPressureLoss))
print('R17 Modal Pressure Targets : ' + str(args.ModalPressureTargets))
...
if args.ModalPressureLoss:
    repertoire_new = repertoire_new + '_PMODBAL'
...
# R17 modal pressure targets: one row per physical pressure tap.
x_tf_pmodal_cyl = tf.compat.v1.placeholder(dtype=tf.float32,shape=[None,1])
y_tf_pmodal_cyl = tf.compat.v1.placeholder(dtype=tf.float32,shape=[None,1])
pmodal_re_tf = tf.compat.v1.placeholder(dtype=tf.float32,shape=[None,Nmodes])
pmodal_im_tf = tf.compat.v1.placeholder(dtype=tf.float32,shape=[None,Nmodes])
pmodal_denom_tf = tf.compat.v1.placeholder(dtype=tf.float32,shape=[Nmodes])
...
def loss_pressure_modes_normalized(x,y,target_re,target_im,denom):
    """
    R17 equalized modal pressure objective.

    fluid_p follows the code convention
        p(t) = Re[sum_k p_hat_k exp(i*k*omega*t)].

    k=0:
        only Re(p_hat_0) is observable in real pressure; Im(p_hat_0) is
        deliberately ignored so R17 does not constrain a gauge/null direction
        that the original time-domain pressure loss cannot see either.

    k>=1:
        both real and imaginary components are observable.

    Returns
    -------
    scalar_raw : mean over modes of normalized squared modal error
    per_mode    : [Nmodes] normalized squared modal errors
    """
    pred = fluid_p(x,y)[0,:,:]  # [Ntaps,Nmodes], complex64
    target = tf.complex(target_re,target_im)

    errs=[]
    for k in range(Nmodes):
        if k == 0:
            e2 = tf.square(tf.real(pred[:,k])-target_re[:,k])
        else:
            d = pred[:,k]-target[:,k]
            e2 = tf.square(tf.real(d))+tf.square(tf.imag(d))
        errs.append(tf.reduce_mean(e2)/(denom[k]+1.e-30))

    per_mode=tf.stack(errs)
    return tf.reduce_mean(per_mode),per_mode
...
# R17: normalized modal pressure error. This graph is tiny: it evaluates only
# the pressure network at the physical tap locations (32 points), with NO
# Navier--Stokes derivatives.
Loss_mes_cyl_modal_raw,Loss_mes_cyl_modal_per_mode = \
    loss_pressure_modes_normalized(
        x_tf_pmodal_cyl,y_tf_pmodal_cyl,
        pmodal_re_tf,pmodal_im_tf,pmodal_denom_tf)

# Fixed non-trainable scalars assigned once after the pressure targets are
# loaded and the common R10 warm start is evaluated.
modal_pressure_lambda_var=tf.Variable(
    0.0,dtype=tf.float32,trainable=False,name='modal_pressure_lambda')
modal_pressure_irreducible_var=tf.Variable(
    0.0,dtype=tf.float32,trainable=False,name='modal_pressure_irreducible')

modal_pressure_lambda_ph=tf.compat.v1.placeholder(
    tf.float32,shape=[],name='modal_pressure_lambda_new')
modal_pressure_irreducible_ph=tf.compat.v1.placeholder(
    tf.float32,shape=[],name='modal_pressure_irreducible_new')

modal_pressure_lambda_assign=tf.compat.v1.assign(
    modal_pressure_lambda_var,modal_pressure_lambda_ph)
modal_pressure_irreducible_assign=tf.compat.v1.assign(
    modal_pressure_irreducible_var,modal_pressure_irreducible_ph)

Loss_mes_cyl_modal = (
    modal_pressure_lambda_var*Loss_mes_cyl_modal_raw
    + modal_pressure_irreducible_var
)
...
if args.PressureOnly:
    # Pressure-only mode: cylinder-surface pressure taps only.
    if args.ModalPressureLoss:
        Loss_mes_exp = Loss_mes_cyl_modal
    else:
...
# =============================================================================
# R17 modal pressure targets and one-time scale calibration
# =============================================================================
_r17_modal_info=None
if args.ModalPressureLoss:
    _mt=np.load(args.ModalPressureTargets)

    _required=[
      'x_taps','y_taps','times','p_raw','p_fit',
      'target_real','target_imag','denom',
      'truncation_mse','omega','Nmodes','NTaps'
    ]
    for _k in _required:
        if _k not in _mt:
            raise ValueError('R17 target file missing key: '+_k)

    if int(_mt['Nmodes']) != Nmodes:
        raise ValueError(
          'R17 target Nmodes=%d but training Nmodes=%d'%
          (int(_mt['Nmodes']),Nmodes))
    if int(_mt['NTaps']) != args.NTaps:
        raise ValueError(
          'R17 target NTaps=%d but training NTaps=%d'%
          (int(_mt['NTaps']),args.NTaps))
    if abs(float(_mt['omega'])-omega_0) > 1.e-8:
        raise ValueError(
          'R17 target omega %.12g != model omega %.12g'%
          (float(_mt['omega']),omega_0))

    _xm=_mt['x_taps'].astype(np.float32).reshape(-1,1)
    _ym=_mt['y_taps'].astype(np.float32).reshape(-1,1)
    _pr=_mt['target_real'].astype(np.float32)
    _pi=_mt['target_imag'].astype(np.float32)
    _pd=_mt['denom'].astype(np.float32)
    _irreducible=float(_mt['truncation_mse'])
    _times_target=_mt['times'].astype(np.float64).reshape(-1)
    _praw_target=_mt['p_raw'].astype(np.float64)
    _pfit_target=_mt['p_fit'].astype(np.float64)

    # R17 must be built from exactly the same physical pressure observations
    # that the R15 time-domain objective sees.  Verify coordinates, times and
    # raw pressures after the training loader has flattened them.
    _ntarget=len(_times_target)
    if _praw_target.shape != (_ntarget,args.NTaps):
        raise ValueError('R17 p_raw target shape mismatch: '+str(_praw_target.shape))
    if _pfit_target.shape != (_ntarget,args.NTaps):
        raise ValueError('R17 p_fit target shape mismatch: '+str(_pfit_target.shape))
    if Ncyl != _ntarget*args.NTaps:
        raise ValueError(
          'R17 training cylinder sample count %d != target %d x %d'%
          (Ncyl,_ntarget,args.NTaps))

    _xtrain=np.asarray(xmes_cyl,dtype=np.float64).reshape(_ntarget,args.NTaps)
    _ytrain=np.asarray(ymes_cyl,dtype=np.float64).reshape(_ntarget,args.NTaps)
    _ttrain=np.asarray(tmes_cyl,dtype=np.float64).reshape(_ntarget,args.NTaps)
    _ptrain=np.asarray(pmes_cyl,dtype=np.float64).reshape(_ntarget,args.NTaps)

    _coord_err=max(
      float(np.max(np.abs(_xtrain-_xm.reshape(1,-1)))),
      float(np.max(np.abs(_ytrain-_ym.reshape(1,-1)))))
    _time_err=float(np.max(np.abs(
      _ttrain-_times_target.reshape(-1,1))))
    _pressure_data_err=float(np.max(np.abs(_ptrain-_praw_target)))

    if _coord_err > 2.e-6:
        raise RuntimeError('R17 target/training tap-coordinate mismatch: %.3e'%_coord_err)
    if _time_err > 1.e-10:
        raise RuntimeError('R17 target/training time mismatch: %.3e'%_time_err)
    if _pressure_data_err > 1.e-10:
        raise RuntimeError('R17 target/training pressure mismatch: %.3e'%_pressure_data_err)
    ...
    if isinstance(tf_dict,list):
        for _d in tf_dict:
            _d.update(_modal_feed)
        _cal_feed=tf_dict[0]
    else:
        tf_dict.update(_modal_feed)
        _cal_feed=tf_dict

    # R15's time-domain loss can be decomposed exactly on these discrete tap
    # samples into a trainable projection term plus the orthogonal LS residual.
    _time0,_modal0,_per0=sess.run(
        [Loss_mes_cyl,Loss_mes_cyl_modal_raw,
         Loss_mes_cyl_modal_per_mode],
        feed_dict=_cal_feed)

    _trainable_time0=float(_time0)-_irreducible
    if not np.isfinite(_modal0) or float(_modal0)<=0.:
        raise RuntimeError('R17 initial normalized modal pressure loss is invalid.')
    if not np.isfinite(_irreducible) or _irreducible < 0.:
        raise RuntimeError('R17 truncation MSE is invalid.')
    if _trainable_time0 <= 0.:
        raise RuntimeError(
          'R17 projected trainable pressure loss is non-positive: time=%.8e irreducible=%.8e'%
          (float(_time0),_irreducible))

    # Independent numerical reconstruction at the common warm start.  This
    # checks, in one place, the tap ordering, the no-factor-2 Fourier
    # convention, and the LS orthogonal decomposition used for calibration.
    _pred_modes0=sess.run(
        fluid_p(x_tf_pmodal_cyl,y_tf_pmodal_cyl)[0,:,:],
        feed_dict=_cal_feed).astype(np.complex128)
    _ppred0=np.zeros_like(_praw_target,dtype=np.float64)
    for _kk in range(Nmodes):
        _ppred0 += np.real(
          _pred_modes0[:,_kk][None,:] *
          np.exp(1j*_kk*omega_0*_times_target[:,None]))

    _raw_mse_np=float(np.mean((_ppred0-_praw_target)**2))
    _proj_mse_np=float(np.mean((_ppred0-_pfit_target)**2))
    _decomp_mse_np=float(_proj_mse_np+_irreducible)

    _raw_graph_rel=abs(_raw_mse_np-float(_time0))/max(abs(float(_time0)),1.e-30)
    _decomp_rel=abs(_decomp_mse_np-_raw_mse_np)/max(abs(_raw_mse_np),1.e-30)
    _proj_rel=abs(_proj_mse_np-_trainable_time0)/max(abs(_trainable_time0),1.e-30)

    if _raw_graph_rel > 2.e-5:
        raise RuntimeError('R17 graph-vs-modal reconstruction MSE mismatch: %.3e'%_raw_graph_rel)
    if _decomp_rel > 2.e-10:
        raise RuntimeError('R17 LS orthogonal-decomposition check failed: %.3e'%_decomp_rel)
    if _proj_rel > 2.e-5:
        raise RuntimeError('R17 projected-loss calibration check failed: %.3e'%_proj_rel)

    _lambda=float(_trainable_time0/float(_modal0))

    sess.run(
      [modal_pressure_lambda_assign,modal_pressure_irreducible_assign],
      feed_dict={
        modal_pressure_lambda_ph:np.float32(_lambda),
        modal_pressure_irreducible_ph:np.float32(_irreducible),
      })

    _weighted0=sess.run(Loss_mes_cyl_modal,feed_dict=_cal_feed)
    _relmatch=abs(float(_weighted0)-float(_time0))/max(abs(float(_time0)),1.e-30)
    if _relmatch > 2.e-5:
        raise RuntimeError(
          'R17 calibration mismatch: weighted modal %.8e vs time %.8e'%
          (_weighted0,_time0))

    _r17_modal_info={
      'time_domain_pressure_loss_initial':float(_time0),
      'truncation_mse_constant':float(_irreducible),
      'trainable_projected_time_loss_initial':float(_trainable_time0),
      'raw_normalized_modal_loss_initial':float(_modal0),
      'raw_normalized_per_mode_initial':[float(z) for z in _per0],
      'modal_lambda_fixed':float(_lambda),
      'weighted_modal_plus_constant_initial':float(_weighted0),
      'relative_initial_scale_match':float(_relmatch),
      'target_training_coordinate_max_abs_diff':float(_coord_err),
      'target_training_time_max_abs_diff':float(_time_err),
      'target_training_pressure_max_abs_diff':float(_pressure_data_err),
      'warmstart_raw_graph_vs_modal_reconstruction_rel':float(_raw_graph_rel),
      'warmstart_ls_decomposition_rel':float(_decomp_rel),
      'warmstart_projected_loss_rel':float(_proj_rel),
    }

    print('R17 MODAL PRESSURE CALIBRATION:')
    print(_r17_modal_info)
    with open(repertoire+'/R17_modal_pressure_calibration.json','w') as _f:
        import json as _json
        _json.dump(_r17_modal_info,_f,indent=2)
...
if _r14_ckpt_targets:
    ...
    print('R17 saved accepted-step checkpoint iter=0')
    ...
    def _r14_accepted_step_callback(xk):
        ...
        if ii in _r14_ckpt_targets:
            ...
            print('R17 CHECKPOINT accepted_iter=%d saved'%ii)
            sys.stdout.flush()
else:
...
if _r14_ckpt_targets:
    ...
    print('R17 accepted L-BFGS iterations:',_r14_final_iter)
    ...
    if _r14_state['last_xk'] is not None:
        ...
        print('R17 final session-flat vs last accepted xk relative difference = %.3e'%_rel)
        if _rel > 5.e-5:
            raise RuntimeError('R17 accepted-step packed-vector mapping check failed.')
    ...
    if not os.path.exists(_final_flat_path):
        ...
        print('R17 saved additional final checkpoint iter=%d'%_r14_final_iter)
    ...
    for _ii,_ff in _items:
        ...
        if args.ModalPressureLoss:
            _mr,_mp=sess.run(
                ...
                feed_dict=_feed)
            _r17_ckpt_extra={
              'modal_pressure_raw_normalized':float(_mr),
              'modal_pressure_per_mode_normalized_sq':[float(z) for z in _mp],
            }
        ...
        _entry.update(_r17_ckpt_extra)
        _manifest['checkpoints'].append(_entry)
        print('R17 converted checkpoint iter=%d total=%.6e physics=%.6e taps=%.6e'%
              (_ii,_tot,_phy,_tap))
    ...
    print('R17 checkpoint manifest saved:',
...
_r11_summary = {
    ...
    'modal_pressure_loss': bool(args.ModalPressureLoss),
}
if args.ModalPressureLoss:
    _r17_raw,_r17_per,_r17_lam,_r17_irr=sess.run(
        [Loss_mes_cyl_modal_raw,Loss_mes_cyl_modal_per_mode,
         modal_pressure_lambda_var,modal_pressure_irreducible_var],
        feed_dict=_eval_dict)
    _r11_summary.update({
      'modal_pressure_raw_normalized':float(_r17_raw),
      'modal_pressure_per_mode_normalized_sq':[float(z) for z in _r17_per],
      'modal_pressure_lambda':float(_r17_lam),
      'modal_pressure_irreducible_constant':float(_r17_irr),
    })
