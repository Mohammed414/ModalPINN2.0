...
def gen_int_random_points(Npoints,geom,method = 'uniform', disp_plot = False, Delta_r_c = 0.5):
    # Generate Npoints inside the rectangular fluid domain excluding the cylinder.
    #
    # R12 wake_biased mixture:
    #   30% whole-domain uniform
    #   35% formation wake: 0 <= x < 3, |y| <= 2
    #   25% far wake:       3 <= x <= 8, |y| <= 2
    #   10% near-cylinder annulus: 0.5 < r <= 1.2
    #
    # This intentionally changes the Monte-Carlo weighting of the physics
    # objective toward the wake. It is NOT importance-corrected.
    ...
    if method == 'wake_biased':
        ...
        # Area-uniform annulus: r^2 is uniform.
        r_outer = 1.2
        ...
        ok = ((x3 >= Lxmin)&(x3 <= Lxmax)&(y3 >= Lymin)&(y3 <= Lymax)&
              (((x3-x_c)**2+(y3-y_c)**2) > r_c**2))
        if not np.all(ok):
            raise RuntimeError('Wake-biased annulus unexpectedly left domain.')
        ...
        print('R12 source mixture counts: whole=%d formation=%d far=%d annulus=%d' %
    ...
    if disp_plot:
        ...
        plt.xlabel('$x$')
        plt.ylabel('$y$')
        plt.title('Random training points generation: '+method)
        plt.tight_layout()
