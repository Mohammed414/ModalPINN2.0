...
parser.add_argument('--InteriorPointsFile',type=str,default=None,help="R19: override the generated interior x,y,t with arrays from an npz file. Requires exactly Nint points and --multigrid --Ngrid 1.")
...
print('R19 Interior Points File : ' + str(args.InteriorPointsFile))
...
if args.InteriorPointsFile is not None:
    if not args.multigrid or args.Ngrid != 1:
        raise ValueError('--InteriorPointsFile requires --multigrid --Ngrid 1.')
    if not os.path.exists(args.InteriorPointsFile):
        raise IOError('Interior points file not found: '+args.InteriorPointsFile)
...
if args.InteriorPointsFile is not None:
    repertoire_new = repertoire_new + '_R19CUSTOMVIN'
if args.BVF:
...
if args.SparseData:
    ...
    if args.InteriorPointsFile is not None:
        _vin=np.load(args.InteriorPointsFile)
        for _k in ('x','y','t'):
            if _k not in _vin:raise ValueError('Interior file missing '+_k)
        _xv=np.asarray(_vin['x'],dtype=np.float64).reshape(-1)
        _yv=np.asarray(_vin['y'],dtype=np.float64).reshape(-1)
        _tv=np.asarray(_vin['t'],dtype=np.float64).reshape(-1)
        if not (len(_xv)==len(_yv)==len(_tv)==Nint):
            raise ValueError('Interior file must contain exactly Nint triplets.')
        if np.any((_xv-x_c)**2+(_yv-y_c)**2 <= r_c**2):
            raise ValueError('Interior file contains point inside/on cylinder.')
        if np.any(_xv<Lxmin) or np.any(_xv>Lxmax) or np.any(_yv<Lymin) or np.any(_yv>Lymax):
            raise ValueError('Interior file contains out-of-domain point.')
        if np.any(_tv<0.) or np.any(_tv>1.e2):
            raise ValueError('Interior times must lie in [0,100].')
        x_int=[_xv];y_int=[_yv];t_int=[_tv]
        print('R19 loaded custom interior set from %s with %d points'%(args.InteriorPointsFile,Nint))
        if 'n_fixed' in _vin and 'n_adaptive' in _vin:
            print('R19 fixed/adaptive = %d / %d'%(int(_vin['n_fixed']),int(_vin['n_adaptive'])))
    Ncyl = len(xmes_cyl)
...
if _r14_ckpt_targets:
    ...
    print('R19 saved accepted-step checkpoint iter=0')
    ...
    def _r14_accepted_step_callback(xk):
        ...
        if ii in _r14_ckpt_targets:
            ...
            print('R19 CHECKPOINT accepted_iter=%d saved'%ii)
            sys.stdout.flush()
else:
...
if _r14_ckpt_targets:
    ...
    print('R19 accepted L-BFGS iterations:',_r14_final_iter)
    ...
    if _r14_state['last_xk'] is not None:
        ...
        print('R19 final session-flat vs last accepted xk relative difference = %.3e'%_rel)
        if _rel > 5.e-5:
            raise RuntimeError('R19 accepted-step packed-vector mapping check failed.')
    ...
    if not os.path.exists(_final_flat_path):
        ...
        print('R19 saved additional final checkpoint iter=%d'%_r14_final_iter)
    ...
    for _ii,_ff in _items:
        ...
        _entry={
            ...
            # Diagnostics run outside the training subprocess cwd, so persist
            # absolute checkpoint paths in the manifest.
        ...
        print('R19 converted checkpoint iter=%d total=%.6e physics=%.6e taps=%.6e'%
              (_ii,_tot,_phy,_tap))
    ...
    print('R19 checkpoint manifest saved:',
...
_r11_summary = {
    ...
    'interior_points_file': args.InteriorPointsFile,
}
