...
parser.add_argument('--ModalBVF',action="store_true",default=False,help="R18: impose the Lighthill boundary-vorticity-flux relation directly on the Fourier modes at the 32 cylinder taps. Targets are derived only from pressure taps. No time expansion.")
parser.add_argument('--ModalBVFTargets',type=str,default=None,help="R18 .npz target file produced by build_modal_bvf_targets.py.")
parser.add_argument('--ModalBVFTargetFrac',type=float,default=0.10,help="R18b: warm-start calibration target for lambda_BVF*L_BVF_raw as a fraction of the initial ordinary NS physics loss.")
parser.add_argument('--ModalBVFH',type=float,default=0.02,help="R18b: outward radial step h/D for the second-order one-sided finite-difference approximation of d(omega)/dn.")
...
parser.add_argument('--SkipValidationData',action="store_true",default=False,help="Operational speed flag: do not build/load the legacy 10x validation dataset. It is not part of the optimized loss and R17-R19 use independent post-training audits instead.")
...
if args.ModalBVF:
    if args.ModalBVFTargets is None:
        raise ValueError('--ModalBVF requires --ModalBVFTargets <npz>.')
    if args.ModalBVFTargetFrac <= 0.0:
        raise ValueError('--ModalBVFTargetFrac must be positive.')
    if args.ModalBVFH <= 0.0:
        raise ValueError('--ModalBVFH must be positive.')
    if args.BVF:
        raise ValueError('Use either legacy --BVF or R18b --ModalBVF, not both.')
...
print('R18b FD Modal BVF : ' + str(args.ModalBVF))
print('R18b FD Modal BVF Targets : ' + str(args.ModalBVFTargets))
print('R18b FD Modal BVF target fraction : %.4g' % args.ModalBVFTargetFrac)
print('R18b FD Modal BVF h/D : %.6g' % args.ModalBVFH)
...
if args.ModalBVF:
    repertoire_new = repertoire_new + '_MODALBVF_FD_h%s' % str(args.ModalBVFH).replace('.', 'p')
...
r_c = 0.5 # radius
if args.ModalBVF and r_c + 2.0*args.ModalBVFH >= 1.2:
    raise ValueError('--ModalBVFH is too large for the near-wall FD stencil.')
d = 2.*r_c
u_in = 1. 
rho_0 = 1.
...
# R18b FD modal-BVF wall targets.  Each row is one physical wall tap and each
# column is one temporal Fourier mode.
# R18b v6 FAST: stack R, R+h and R+2h into ONE 96-point graph.
# This is mathematically identical to the previous three-graph implementation,
# but avoids triplicating the u/v modal network and derivative graph.
x_tf_mbvf_fd = tf.compat.v1.placeholder(dtype=tf.float32,shape=[None,1])
y_tf_mbvf_fd = tf.compat.v1.placeholder(dtype=tf.float32,shape=[None,1])
g_re_tf_mbvf = tf.compat.v1.placeholder(dtype=tf.float32,shape=[None,Nmodes])
g_im_tf_mbvf = tf.compat.v1.placeholder(dtype=tf.float32,shape=[None,Nmodes])
g_den_tf_mbvf = tf.compat.v1.placeholder(dtype=tf.float32,shape=[Nmodes])
...
def loss_modal_bvf_fd(x,y,g_re,g_im,g_den,h,nwall,residual_clip=50.):
    """
    R18b v6 fused finite-difference modal Lighthill BVF.

    x,y contain one stacked batch:
        [ r=R taps ; r=R+h taps ; r=R+2h taps ]

    This is exactly the same second-order stencil as v5, but the modal u/v
    networks are instantiated only once for all 96 points rather than three
    separate times.  k=0 imaginary vorticity is not built because the real
    flow cannot observe it and the loss never uses it.
    """
    um=fluid_u(x,y)[0,:,:]
    vm=fluid_v(x,y)[0,:,:]

    htf=tf.cast(h,tf.float32)
    per_mode=[]

    for k in range(Nmodes):
        ur=tf.real(um[:,k:k+1])
        vr=tf.real(vm[:,k:k+1])
        wr=tf.gradients(vr,x)[0]-tf.gradients(ur,y)[0]

        wr0=wr[0:nwall,:]
        wr1=wr[nwall:2*nwall,:]
        wr2=wr[2*nwall:3*nwall,:]
        dwdn_r=(-3.0*wr0+4.0*wr1-wr2)/(2.0*htf)

        rr=(1./Re)*dwdn_r-g_re[:,k:k+1]
        rr=tf.clip_by_value(rr,-residual_clip,residual_clip)
        ...
        if k == 0:
            ...
            ui=tf.imag(um[:,k:k+1])
            vi=tf.imag(vm[:,k:k+1])
            wi=tf.gradients(vi,x)[0]-tf.gradients(ui,y)[0]

            wi0=wi[0:nwall,:]
            wi1=wi[nwall:2*nwall,:]
            wi2=wi[2*nwall:3*nwall,:]
            dwdn_i=(-3.0*wi0+4.0*wi1-wi2)/(2.0*htf)

            ri=(1./Re)*dwdn_i-g_im[:,k:k+1]
            ri=tf.clip_by_value(ri,-residual_clip,residual_clip)
            e2=tf.square(rr)+tf.square(ri)

        per_mode.append(tf.reduce_mean(e2)/(g_den[k]+1.e-30))

    per_mode=tf.stack(per_mode)
...
# R18 modal BVF.  The scalar lambda is calibrated once at the common warm
# start and then remains fixed.
Loss_mbvf_raw=tf.constant(0.0,dtype=tf.float32)
Loss_mbvf_per_mode=tf.zeros([Nmodes],dtype=tf.float32)
mbvf_lambda_var=tf.Variable(
    0.0,dtype=tf.float32,trainable=False,name='modal_bvf_lambda')
mbvf_lambda_ph=tf.compat.v1.placeholder(
    tf.float32,shape=[],name='modal_bvf_lambda_new')
mbvf_lambda_assign=tf.compat.v1.assign(mbvf_lambda_var,mbvf_lambda_ph)

if args.ModalBVF:
    Loss_mbvf_raw,Loss_mbvf_per_mode=loss_modal_bvf_fd(
        x_tf_mbvf_fd,y_tf_mbvf_fd,
        g_re_tf_mbvf,g_im_tf_mbvf,g_den_tf_mbvf,
        args.ModalBVFH,args.NTaps)
...
if args.ModalBVF:
    Loss = Loss + mbvf_lambda_var * Loss_mbvf_raw
...
if args.SkipAdam:
    opt_Adam = None
    print('Adam graph NOT declared (--SkipAdam)')
    print_mem('after skipping Adam declaration')
else:
...
# =============================================================================
# R18b FD modal-BVF target feed and one-time lambda calibration
# =============================================================================
_r18_mbvf_info=None
if args.ModalBVF:
    _mz=np.load(args.ModalBVFTargets)
    for _key in [
        'x_wall','y_wall','g_real','g_imag','denom',
        'omega','Nmodes','NTaps',
        'source_x','source_y','source_t','source_p'
    ]:
        if _key not in _mz:
            raise ValueError('R18b target file missing key: '+_key)

    if int(_mz['Nmodes']) != Nmodes:
        raise ValueError('R18b target Nmodes mismatch.')
    if int(_mz['NTaps']) != args.NTaps:
        raise ValueError('R18b target NTaps mismatch.')
    if abs(float(_mz['omega'])-omega_0)>1.e-8:
        raise ValueError('R18b target omega mismatch.')

    _sx=np.asarray(_mz['source_x'],dtype=np.float64).reshape(-1)
    _sy=np.asarray(_mz['source_y'],dtype=np.float64).reshape(-1)
    _st=np.asarray(_mz['source_t'],dtype=np.float64).reshape(-1)
    _sp=np.asarray(_mz['source_p'],dtype=np.float64).reshape(-1)
    _tx=np.asarray(xmes_cyl,dtype=np.float64).reshape(-1)
    _ty=np.asarray(ymes_cyl,dtype=np.float64).reshape(-1)
    _tt=np.asarray(tmes_cyl,dtype=np.float64).reshape(-1)
    _tp=np.asarray(pmes_cyl,dtype=np.float64).reshape(-1)
    if not (_sx.size==_sy.size==_st.size==_sp.size==_tx.size==_ty.size==_tt.size==_tp.size):
        raise RuntimeError('R18b target/training pressure sample count mismatch.')
    for _name,_a,_b in [('x',_sx,_tx),('y',_sy,_ty),('t',_st,_tt),('p',_sp,_tp)]:
        if not np.allclose(_a,_b,rtol=0.0,atol=2.e-8):
            raise RuntimeError('R18b target/training pressure mismatch in %s: max abs %.3e'%(_name,float(np.max(np.abs(_a-_b)))))
    print('R18b pressure-source identity check PASS: %d samples.'%_sx.size)

    _x0=_mz['x_wall'].astype(np.float32).reshape(-1,1)
    _y0=_mz['y_wall'].astype(np.float32).reshape(-1,1)
    _r0=np.sqrt(_x0*_x0+_y0*_y0)
    if len(_x0) != args.NTaps:
        raise ValueError('R18b target wall-point count mismatch.')
    if np.max(np.abs(_r0-r_c)) > 5.e-6:
        raise ValueError('R18b target wall points are not on r=R.')
    _gre=np.asarray(_mz['g_real'],dtype=np.float32)
    _gim=np.asarray(_mz['g_imag'],dtype=np.float32)
    _gden=np.asarray(_mz['denom'],dtype=np.float32).reshape(-1)
    if _gre.shape != (args.NTaps,Nmodes) or _gim.shape != (args.NTaps,Nmodes):
        raise ValueError('R18b modal BVF target shape mismatch.')
    if _gden.shape != (Nmodes,) or np.any(_gden<=0.) or not np.all(np.isfinite(_gden)):
        raise ValueError('R18b modal BVF normalization denominator invalid.')
    if not np.all(np.isfinite(_gre)) or not np.all(np.isfinite(_gim)):
        raise ValueError('R18b modal BVF target contains non-finite values.')

    _s1=np.float32((r_c+args.ModalBVFH)/r_c)
    _s2=np.float32((r_c+2.0*args.ModalBVFH)/r_c)

    _xfd=np.concatenate([_x0,_s1*_x0,_s2*_x0],axis=0)
    _yfd=np.concatenate([_y0,_s1*_y0,_s2*_y0],axis=0)
    if len(_xfd) != 3*args.NTaps:
        raise RuntimeError('R18b fused FD coordinate count mismatch.')
    ...
    if isinstance(tf_dict,list):
        for _d in tf_dict:
            _d.update(_mbvf_feed)
        _cal_feed=tf_dict[0]
    else:
        tf_dict.update(_mbvf_feed)
        _cal_feed=tf_dict

    _phys0,_raw0,_per0=sess.run(
        [Loss_int_time_wrap,Loss_mbvf_raw,Loss_mbvf_per_mode],
        feed_dict=_cal_feed)

    if not np.isfinite(_raw0) or float(_raw0)<=0.:
        raise RuntimeError('R18b initial modal-BVF raw loss invalid.')

    _lambda=float(
        args.ModalBVFTargetFrac*float(_phys0)/float(_raw0)
    )
    sess.run(
        mbvf_lambda_assign,
        feed_dict={mbvf_lambda_ph:np.float32(_lambda)}
    )
    _weighted0=float(_lambda)*float(_raw0)
    _frac0=_weighted0/max(abs(float(_phys0)),1.e-30)
    if not np.isfinite(_lambda) or _lambda<=0.:
        raise RuntimeError('R18b calibrated modal-BVF lambda is invalid.')
    if abs(_frac0-float(args.ModalBVFTargetFrac)) > 2.e-6:
        raise RuntimeError('R18b BVF calibration fraction mismatch: got %.8g expected %.8g'%(_frac0,args.ModalBVFTargetFrac))

    _r18_mbvf_info={
      'physics_loss_initial':float(_phys0),
      'modal_bvf_raw_initial':float(_raw0),
      'modal_bvf_per_mode_initial':[float(z) for z in _per0],
      'modal_bvf_lambda_fixed':float(_lambda),
      'modal_bvf_weighted_initial':float(_weighted0),
      'achieved_fraction_of_initial_physics':float(_frac0),
      'target_fraction_of_initial_physics':
          float(args.ModalBVFTargetFrac),
      'finite_difference_h_over_D':float(args.ModalBVFH),
      'finite_difference_order':2,
      'finite_difference_formula':'[-3*w(R)+4*w(R+h)-w(R+2h)]/(2h)',
    }
    print('R18b FD MODAL BVF CALIBRATION:')
    print(_r18_mbvf_info)
    with open(repertoire+'/R18b_modal_bvf_fd_calibration.json','w') as _f:
        import json as _json
        _json.dump(_r18_mbvf_info,_f,indent=2)



# Legacy validation data are monitoring-only; they never enter Loss.
# R18b uses independent post-training fixed-uniform audits, so avoid spending
# minutes rereading CFD and generating 250k unused points in the smoke/full run.
if args.SkipValidationData:
    print('Skipping legacy 10x validation data (--SkipValidationData).')
    tf_dict_valid=None
else:
    print('Loading validation data set')
    x_int_valid,y_int_valid,t_int_valid,s_train,xmes_valid,ymes_valid,tmes_valid,umes_valid,vmes_valid,pmes_valid = ltd.training_dict(
        10*Nmes,10*Nint,Nbc,filename_data,geom,Tintmax=1e2,
        cut=True,method_int='uniform')
    Nmesvalid = len(xmes_valid)
    tf_dict_valid = {
      x_tf_int:np.reshape(x_int_valid,(10*Nint,1)),
...
print('R18b PRE_LBFGS_READY elapsed_s=%.3f'%(time.time()-t0))
sys.stdout.flush()
...
if _r14_ckpt_targets:
    ...
    print('R18b saved accepted-step checkpoint iter=0')
    ...
    def _r14_accepted_step_callback(xk):
        ...
        if ii in _r14_ckpt_targets:
            ...
            print('R18b CHECKPOINT accepted_iter=%d saved'%ii)
            sys.stdout.flush()
else:
...
if _r14_ckpt_targets:
    ...
    print('R18b accepted L-BFGS iterations:',_r14_final_iter)
    ...
    if _r14_state['last_xk'] is not None:
        ...
        print('R18b final session-flat vs last accepted xk relative difference = %.3e'%_rel)
        if _rel > 5.e-5:
            raise RuntimeError('R18b accepted-step packed-vector mapping check failed.')
    ...
    if not os.path.exists(_final_flat_path):
        ...
        print('R18b saved additional final checkpoint iter=%d'%_r14_final_iter)
    ...
    for _ii,_ff in _items:
        ...
        _r18_extra={}
        if args.ModalBVF:
            _br,_bp,_bl=sess.run(
                [Loss_mbvf_raw,Loss_mbvf_per_mode,mbvf_lambda_var],
                feed_dict=_feed)
            _r18_extra={
              'modal_bvf_raw':float(_br),
              'modal_bvf_per_mode':[float(z) for z in _bp],
              'modal_bvf_lambda':float(_bl),
              'modal_bvf_weighted':float(_bl*_br),
            }
        ...
        _entry={
            ...
            'model_dir':os.path.abspath(_dir),
            'model_file':os.path.abspath(_dir+'/'+_dnn_name),
            'training_total_loss':float(_tot),
            'training_physics_loss':float(_phy),
            'training_pressure_tap_loss':float(_tap),
        }
        _entry.update(_r18_extra)
        _manifest['checkpoints'].append(_entry)
        print('R18b converted checkpoint iter=%d total=%.6e physics=%.6e taps=%.6e'%
              (_ii,_tot,_phy,_tap))
    ...
    print('R18b checkpoint manifest saved:',
