...
parser.add_argument('--LambdaK0',type=float,default=0.5,help="R6: weight of the k=0 harmonic momentum residual loss term when --K0Loss is set. R5's default (0.025) was measured post-hoc (R6 plan audit) to have been 3-10x below the weight actually needed to outbid the wall-pitot/BVF regression cost of fixing the wake (break-even at w~0.082) - R5's term was outbid, not defeated. 0.5 is the audit's high end, giving the term real authority (~30% of the objective at that weight) while still leaving wall-pressure/BVF enough weight to keep contesting the mean-flow base-flow drift risk (see the periodic drift diagnostic below).")
...
parser.add_argument('--WarmStartFrom',type=str,default=None,help="R6: path to a previous run's DNN2_*_tanh.pickle to warm-start weights from (trainable tf.Variables, not a frozen restore - restore_NN's tf_as_constant=False) instead of random Xavier init. Architecture (--WidthLayer/--Nmodes here must match the checkpoint's) is asserted at load time, not assumed. Motivation (R6 plan): R3 already has a correct mean flow and near-field fit - training from scratch makes the new k=0 term compete against an untrained network for that, instead of just building on it.")
parser.add_argument('--K0WakeOnly',action="store_true",default=False,help="R6: sample --K0Loss's collocation points from the wake region (x in [1,7], |y|<=2) instead of uniformly over the whole domain. The R6 audit measured this raises the true-field-vs-dead-wake contrast from 9.3x to 10.2x by concentrating gradient where the error actually is, rather than diluting it over the near-cylinder region R3 already solved.")
parser.add_argument('--LBFGSFtol',type=float,default=1e-12,help="R6: ftol passed to declare_LBFGS. R5 used the default (~2.22e-16, machine epsilon) and L-BFGS terminated after only 4574 iterations (vs R3's ~15452) - consistent with a line-search failure at that unusually tight tolerance under the added k=0 term's stiffness, not genuine convergence. Paired with declare_LBFGS's new disp=True so the actual scipy termination message is logged and this can be checked directly rather than inferred from iteration count alone.")
parser.add_argument('--PhaseLoss',action="store_true",default=False,help="R7: dispersion-consistency (wavenumber) residual, added because a phase-0 audit (audit_r7_mistimed_vs_deleted.py, audit_r7_phase_term.py in src/pressure_only/) found that K0Loss/CV1Loss are BOTH exactly, provably invariant to the mode-1 field's spatial phase (proven analytically, confirmed numerically to 5+ sig figs) - neither can ever push the network toward the correct wavenumber, only the tap loss can, and that signal is comparatively weak. This term is normalized by |mode1|^2 so its residual does NOT vanish as amplitude collapses (unlike every existing physics term, including CV1Loss, which is EXACTLY zero at zero amplitude by construction - closes that specific escape hatch). Evaluated on the actual R5 checkpoint's own forward pass, this term came out 631x above the true-field floor, with the checkpoint's own estimated wavenumber essentially flat (0.0024 vs a target of 1.13 implied by its own mean flow) - i.e. it would have flagged exactly the failure R5 produced. See the audit scripts for the full derivation/validation before trusting this at face value.")
parser.add_argument('--LambdaPhase',type=float,default=0.1,help="Weight of --PhaseLoss. UNCALIBRATED against a real training run (this term has never been through TF1.14 before this run) - chosen as a reasonable starting point by rough analogy to LambdaK0/LambdaCV1's calibration process (order-of-magnitude, not measured against this run's own loss components at start). Watch the printed 'Loss phase (component)' value early in training; if it dominates (Loss_phase*LambdaPhase >> other components) or is negligible, this needs adjusting on a follow-up run, not this one.")
parser.add_argument('--PhaseEps',type=float,default=1e-3,help="Fixed regularization floor added to |mode1|^2 in --PhaseLoss's denominator (k_x_est = Im[conj(q1)*dq1/dx]/(|q1|^2+PhaseEps)). MUST be a fixed constant, not derived from the current field's own scale - an earlier version computed eps as a fraction of the CURRENT candidate's own |q1| max and it was a real bug: since q1->s*q1 scales eps by s^2 too, the ratio came out perfectly scale-invariant, silently reproducing the exact amplitude-blindness this term exists to fix (see audit_r7_phase_term.py). This default was calibrated as 0.1*median(true-field |q1|) in the audit (~1.28e-3 there) - not re-derived from this run's own field, which would be circular for the same reason.")
parser.add_argument('--PhaseXMin',type=float,default=3.0,help="Lower x bound for --PhaseLoss's collocation sampling (points drawn from x in [PhaseXMin, Lxmax-0.3], |y|<=2). The underlying dispersion relation (k_x ~= -c*omega_0/u0) was validated against the true CFD field only for x/D >= ~3 - it breaks down inside the recirculation bubble (x < ~2-3), where the mean streamwise velocity u0 changes sign and 'convection speed' isn't physically well-defined. Do not lower this without re-validating there first.")
parser.add_argument('--PhaseC',type=float,default=0.85,help="R8: FIXED value of the convection-deficit scalar c in --PhaseLoss's target k_x = -c*omega_0/u0. R7 made this a trainable tf.Variable specifically to avoid hardcoding the ~0.891 measured against the true field - but that let the optimizer satisfy Loss_phase by collapsing c toward 0 (R7's actual run: c ended at -0.0002) rather than by fixing the network's own wavenumber, driving the target to match whatever flat, near-zero wavenumber the network already had. Confirmed directly on R7's trained checkpoint: wavenumber ratio was 0.4% of truth, WORSE than R5's 13% (no PhaseLoss at all). Fixed here to a constant (tf.constant, not even a non-trainable Variable, so there is no way for it to move) - 0.85 is a round, physically-motivated convection-deficit factor (wake convection speeds running somewhat below the local mean flow is a general, well-known feature of vortex streets, not a number specific to snooping at this dataset's true field), not the exact measured 0.891.")
...
print('Warm Start From : ' + str(args.WarmStartFrom))
print('K0 Wake Only : ' + str(args.K0WakeOnly))
print('LBFGS Ftol : %.4g' % (args.LBFGSFtol))
print('Phase Loss : ' + str(args.PhaseLoss))
print('Lambda Phase : %.4g' % (args.LambdaPhase))
print('Phase Eps : %.4g' % (args.PhaseEps))
print('Phase X Min : %.4g' % (args.PhaseXMin))
print('Phase C (fixed) : %.4g' % (args.PhaseC))
...
if args.K0Loss:
    ...
    if args.K0WakeOnly:
        repertoire_new = repertoire_new + '_wake'
if args.CV1Loss:
    repertoire_new = repertoire_new + '_CV1_lam%s' % (str(args.LambdaCV1).replace('.', 'p'))
if args.HardSym:
    repertoire_new = repertoire_new + '_SYM'
if args.WarmStartFrom:
    repertoire_new = repertoire_new + '_WS'
if args.PhaseLoss:
    repertoire_new = repertoire_new + '_PH_lam%s' % (str(args.LambdaPhase).replace('.', 'p'))
...
# R6 Change 1 (warm start): --WarmStartFrom loads a previous run's (e.g.
# R3's) trained weights as trainable tf.Variables (restore_NN's
# tf_as_constant=False - NOT a frozen restore, these keep training), so the
# new k=0 term builds on an already-correct mean flow/near-field fit instead
# of competing against an untrained network for it. Architecture match is
# asserted, not assumed: a silent shape mismatch here would either crash
# deep inside restore_one_NN with a confusing error, or (worse, if shapes
# happened to be compatible by coincidence) silently load garbage.
if args.WarmStartFrom:
    with open(args.WarmStartFrom, 'rb') as _f:
        _chk_w_u, _chk_b_u, _chk_w_v, _chk_b_v, _chk_w_p, _chk_b_p = pickle.load(_f)
    assert len(_chk_w_u) == len(layers) - 1, (
        'WarmStartFrom checkpoint has %d layers, --WidthLayer/--Nmodes here implies %d' %
        (len(_chk_w_u) + 1, len(layers)))
    for _l in range(len(layers) - 1):
        assert _chk_w_u[_l].shape == (layers[_l], layers[_l + 1]), (
            'WarmStartFrom checkpoint layer %d shape %s does not match --WidthLayer/--Nmodes-implied shape %s' %
            (_l, _chk_w_u[_l].shape, (layers[_l], layers[_l + 1])))
    w_u,b_u,w_v,b_v,w_p,b_p = nnf.restore_NN(layers, args.WarmStartFrom, tf_as_constant=False)
    print('Warm-started weights from %s (architecture check passed)' % args.WarmStartFrom)
else:
...
# R6 Change 5: deliberately NOT tied to --K0Loss here (unlike R5, where
# kill_k0 = bool(args.K0Loss) applied this globally, to every wrapper call -
# BVF, measurements, final reconstruction, everything). R5's audit flagged
# this global gauge fix as confounded with the loss change itself and
# coincident with a 300x BVF regression - it could not be ruled in or out as
# a contributor to R5's failure. R6's primary run keeps the GLOBAL gauge fix
# disabled; k0_residual (below) instead reads only Re(mode 0) LOCALLY, at
# the point of use, without changing what every other loss term or the final
# reconstruction sees. If diagnostics later show gauge drift is a real
# problem, test kill_k0 = bool(args.K0Loss) as its own isolated run.
kill_k0 = False
...
def k0_residual(x, y):
    ...
    R6 Change 5: fluid_u/v/p are called with the GLOBAL kill_k0 gauge fix
    disabled (kill_k0 = False in R6 - see there), so Im(mode 0) is real and
    unconstrained coming out of them. _real_mode0 below re-applies the exact
    same real-projection NN_functions.out_nn_modes_uv/out_nn_modes_p use for
    kill_k0_imag (tf.complex(tf.real(z), 0.*tf.real(z)) - the 0.*real(z), not
    a bare 0., keeps the gradient path connected so d/dx of the discarded
    imaginary part returns a proper zero tensor instead of None), but scoped
    to ONLY this loss term's own local copy of mode 0, rather than the
    global representation every other loss term and the final reconstruction
    read.
    '''
    all_u = fluid_u(x, y)
    all_v = fluid_v(x, y)
    ...
    def _real_mode0(all_modes):
        mode0 = tf.complex(tf.real(all_modes[:, :, 0]), 0. * tf.real(all_modes[:, :, 0]))
        return tf.concat([tf.expand_dims(mode0, 2), all_modes[:, :, 1:]], axis=2)

    all_u = _real_mode0(all_u)
    all_v = _real_mode0(all_v)
    all_p = _real_mode0(all_p)
...
# =============================================================================
# R7 Change: dispersion-consistency (wavenumber) residual, --PhaseLoss.
# See docstring of loss_phase below and audit_r7_mistimed_vs_deleted.py /
# audit_r7_phase_term.py in src/pressure_only/ for the full derivation and
# numpy-level validation this is built from.
# =============================================================================

# R8: FIXED convection-deficit scalar c in k_x_target = -c*omega_0/u0 - a
# tf.constant, not even a non-trainable tf.Variable, so there is no path by
# which any optimizer step can move it.
#
# R7 made this a trainable tf.Variable (trainable=True, picked up
# automatically by declare_LBFGS/declare_Adam's default
# tf.trainable_variables() list) specifically to avoid hardcoding the ~0.891
# measured against the true CFD field. That reasoning had a real bug: the
# residual below is (k_x_est - k_x_target)^2 with k_x_target proportional to
# c, so the optimizer can drive the residual toward zero EITHER by fixing
# the network's own k_x_est to match a genuine target, OR by dragging c
# toward 0 so the target collapses to match whatever (possibly still flat)
# k_x_est the network already had - the exact escape hatch this term exists
# to close, relocated one level up. Confirmed this is what actually
# happened: R7's real run ended with c = -0.0002, and the trained
# checkpoint's own measured wavenumber came out at 0.4% of truth - WORSE
# than R5's 13% (which had no PhaseLoss at all). See --PhaseC's help text.
phase_c_var = tf.constant(args.PhaseC, dtype=tf.float32, name='phase_c')

def loss_phase(x, y):
    '''
    R7: local-wavenumber (dispersion) consistency residual for mode 1.

    Physical basis (validated against the true CFD field for x/D >= ~3
    before being proposed - see audit_r7_phase_term.py): a disturbance
    convecting downstream at speed U_c has phase phi(x) ~= -omega_0*x/U_c,
    so its local streamwise wavenumber k_x = dphi/dx ~= -c*omega_0/u0(x,y),
    with u0 the mean streamwise velocity and c an O(1) convection-deficit
    factor (measured 0.891 +/- 0.021 on the true field, x/D=4-7). Breaks
    down inside the recirculation bubble (u0 changes sign there) - PHASE_X
    below is windowed to avoid that region, mirroring --K0WakeOnly's
    windowing rationale.

    Structural difference from EVERY other physics term in this file
    (loss_int_mode_per_k, k0_residual, _cv1_box_residual): those are all
    built from products/convolutions of the fluctuating modes and are
    therefore quadratic in amplitude - CV1's residual is provably EXACTLY
    zero at zero amplitude (every term needs a nonzero mode-1 factor), and
    k0_residual/CV1 are additionally provably, exactly PHASE-invariant
    (confirmed analytically: their conjugate-paired products cancel any
    common phase rotation exactly). This term instead NORMALIZES by
    |mode1|^2 (with a fixed floor, args.PhaseEps - see its own docstring
    for why this MUST be fixed, not derived from the current field's own
    scale), so its residual does NOT vanish as amplitude collapses: as
    |q1|->0, k_x_est -> 0 (not toward k_x_target), so the residual
    converges to |k_x_target|^2, a fixed nonzero penalty, not zero.

    Returns a tf.float32 tensor of shape [1,Npts]: (k_x_est - k_x_target)^2.
    '''
    all_v = fluid_v(x, y)
    all_u = fluid_u(x, y)
    ...
    one = tf.transpose(0.*x + 1.)
    q1 = all_v[:, :, 1]
    dq1dx = tf.complex(tf.gradients(tf.real(q1), x, grad_ys=one)[0],
                        tf.gradients(tf.imag(q1), x, grad_ys=one)[0])
    u0 = tf.real(all_u[:, :, 0])

    kx_est = tf.imag(tf.conj(q1) * dq1dx) / (nnf.square_norm(q1) + args.PhaseEps)
    kx_target = -phase_c_var * omega_0 / u0
    return tf.square(kx_est - kx_target)
...
# =============================================================================
# R6 Change 6: periodic drift diagnostic (the audit's own kill-switch check).
# Purely a read-only diagnostic - never touches Loss or any trained variable,
# so unlike causal_step_hook this is safe to run during L-BFGS's own
# callback too (which fires on every function evaluation, not just accepted
# steps - mutating anything here would risk the exact bug that broke R4's
# causal weighting under L-BFGS; this never mutates anything).
# =============================================================================
D = 2. * r_c
_drift_x = np.linspace(0.6, 7.0, 60).astype(np.float32).reshape(-1, 1)
_drift_y = np.zeros_like(_drift_x)
_drift_x_tf = tf.constant(_drift_x, dtype=tf.float32)
_drift_y_tf = tf.constant(_drift_y, dtype=tf.float32)
_drift_u_mean_tf = tf.real(fluid_u(_drift_x_tf, _drift_y_tf)[0, :, 0])
DRIFT_CHECK_EVERY = 2000

def drift_step_hook(it):
    '''
    Every DRIFT_CHECK_EVERY iterations, print (a) the wall-pressure tap loss
    (Loss_mes_cyl - a cheap proxy for whether the near-field fit is
    regressing toward the base flow, per the audit's own detector) and (b)
    an approximate recirculation length (first x downstream of the cylinder,
    along y=0, where the k=0 mean-flow u crosses from negative back to
    positive - a crude but cheap probe, not meant to be exact). True field
    ~1.36 D, base (unstable steady) flow ~6.16 D per the audit's numbers -
    if this grows past ~2 D, the plan's pre-committed response is to RAISE
    the wall-pressure weight, not lower --LambdaK0 (that just returns to
    R5's failure mode) - stop the run and restart rather than let it
    complete on autopilot if that threshold is crossed.
    '''
    if it % DRIFT_CHECK_EVERY != 0:
        return
    mes_cyl_feed = tf_dict[0] if multigrid else tf_dict
    wall_p_err = sess.run(Loss_mes_cyl, feed_dict=mes_cyl_feed)
    u_mean = sess.run(_drift_u_mean_tf)
    neg = u_mean < 0
    if neg[0] and np.any(~neg):
        recirc_length = _drift_x[np.argmax(~neg), 0]
    elif neg[0]:
        recirc_length = _drift_x[-1, 0]  # reversed flow all the way to the probe's far edge - at or beyond the danger threshold
    else:
        recirc_length = 0.0  # no reversed flow detected right off the cylinder at all
    print('Drift check @ it %d : wall-pressure tap loss = %.4e, approx recirc length ~ %.3f D' %
          (it, wall_p_err, recirc_length / D))

def combined_step_hook(it):
    causal_step_hook(it)
    drift_step_hook(it)
...
def _build_k0_points(wake_only=False):
    '''
    R6 Change 2: --K0WakeOnly samples directly from the wake region (x in
    [1,7], |y|<=2) instead of masking after uniform whole-domain sampling -
    strictly more sample-efficient (every point contributes gradient signal
    where the R6 audit found it actually lives) for the same K0_N_POINTS
    budget, rather than wasting most of them outside the region of interest.
    '''
    if wake_only:
        xmin, xmax, ymin, ymax = 1.0, 7.0, -2.0, 2.0
    else:
        xmin, xmax, ymin, ymax = Lxmin + 0.5, Lxmax - 0.5, Lymin + 0.5, Lymax - 0.5
    x = _k0_rng.uniform(xmin, xmax, K0_N_POINTS * 2).astype(np.float32)
    y = _k0_rng.uniform(ymin, ymax, K0_N_POINTS * 2).astype(np.float32)
    r = np.sqrt((x - x_c) ** 2 + (y - y_c) ** 2)
    keep = r > 1.5 * r_c
    return x[keep][:K0_N_POINTS].reshape(-1, 1), y[keep][:K0_N_POINTS].reshape(-1, 1)
K0_X, K0_Y = _build_k0_points(wake_only=args.K0WakeOnly)
...
def _build_phase_points():
    '''R7: --PhaseLoss collocation points, x in [PhaseXMin, Lxmax-0.3],
    |y|<=2 - same budget (K0_N_POINTS) and RNG-reuse convention as
    _build_k0_points, windowed per loss_phase's docstring (dispersion
    relation only validated for x/D >= ~3, breaks down in the recirculation
    bubble).'''
    xmin, xmax, ymin, ymax = args.PhaseXMin, Lxmax - 0.3, -2.0, 2.0
    x = _k0_rng.uniform(xmin, xmax, K0_N_POINTS).astype(np.float32)
    y = _k0_rng.uniform(ymin, ymax, K0_N_POINTS).astype(np.float32)
    return x.reshape(-1, 1), y.reshape(-1, 1)

if args.PhaseLoss:
    PHASE_X, PHASE_Y = _build_phase_points()
    x_tf_phase = tf.constant(PHASE_X, dtype=tf.float32)
    y_tf_phase = tf.constant(PHASE_Y, dtype=tf.float32)
    Loss_phase_wrap = tf.reduce_mean(loss_phase(x_tf_phase, y_tf_phase))
...
if args.PhaseLoss:
    Loss = Loss + args.LambdaPhase * Loss_phase_wrap
...
opt_LBFGS = nnf.declare_LBFGS(Loss, ftol=args.LBFGSFtol)
...
List_it_loss_LBFGS,List_it_loss_valid_LBFGS = nnf.model_train_scipy(opt_LBFGS,sess,Loss,tf_dict[0],List_loss = True,tf_dict_valid=tf_dict_valid,loss_valid = Loss_dense_mes,step_hook=combined_step_hook)
...
AdamTmax = Tmax-(t2-t0)
List_it_loss_Adam,List_it_loss_valid_Adam = nnf.model_train_Adam(opt_Adam,sess,Loss,liste_tf_dict=tf_dict,Nit=1e5,tolAdam=1e-5,it=it,itdisp=100,maxTime=AdamTmax,multigrid=multigrid,NgridTurn=NgridTurn,List_loss = True,tf_dict_valid=tf_dict_valid,loss_valid = Loss_dense_mes,step_hook=combined_step_hook)
t3 = time.time()
...
if args.PhaseLoss:
    nnf.tf_print('Loss phase (component)',Loss_phase_wrap,sess,tf_dict[0])
    # No "final value" print for phase_c_var anymore - R8 fixed it to a
    # constant (see its definition above), so it cannot change during
    # training; args.PhaseC (already printed in the Args block) is the
    # whole story now.
