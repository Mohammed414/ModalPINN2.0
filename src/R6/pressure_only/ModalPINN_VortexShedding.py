...
parser.add_argument('--K0WakeOnly',action="store_true",default=False,help="Restrict the k=0 harmonic residual to the wake region x in [1,7] and |y| <= 2 by masking the existing interior collocation points instead of resampling a new k0 grid. Keeps the global path available when disabled.")
parser.add_argument('--K0RampIters',type=int,default=2000,help="Number of iterations over which the k=0 loss weight ramps linearly from 0 to full strength. Uses the same external step-hook scheduling pattern as the existing causal weighting hook.")
...
parser.add_argument('--WarmStartFrom',type=str,default=None,help="Path to a DNN pickle archive used to warm-start the u/v/p mode networks before training. The loaded weights stay trainable; this is a warm start, not a frozen restore.")
parser.add_argument('--LBFGSFtol',type=float,default=1e-12,help="Relative function tolerance passed to L-BFGS-B. Loosened from the default machine-epsilon setting so the optimizer can run through the added k=0 stiffness instead of stopping early.")
...
if args.WarmStartFrom is not None and not os.path.isfile(args.WarmStartFrom):
    raise ValueError('--WarmStartFrom file does not exist: %s' % (args.WarmStartFrom))
...
if args.K0Loss:
    ...
    if args.K0WakeOnly:
        repertoire_new = repertoire_new + '_WakeOnly'
    repertoire_new = repertoire_new + '_Ramp%d' % (args.K0RampIters)
if args.CV1Loss:
    repertoire_new = repertoire_new + '_CV1_lam%s' % (str(args.LambdaCV1).replace('.', 'p'))
if args.WarmStartFrom:
    repertoire_new = repertoire_new + '_WarmStart'
if args.LBFGSFtol != 1e-12:
    repertoire_new = repertoire_new + '_ftol%s' % (('{:.0e}'.format(args.LBFGSFtol)).replace('+', ''))
if args.HardSym:
...
if args.WarmStartFrom is not None:
    expected_layers = [2, args.WidthLayer * Nmodes, args.WidthLayer * Nmodes, Nmodes]
    assert layers == expected_layers, 'Warm-start architecture mismatch: expected %s, got %s' % (expected_layers, layers)
    print('Restoring trainable warm start from %s' % (args.WarmStartFrom))
...
if args.K0Loss:
    k0_ramp_var = tf.Variable(0., dtype=tf.float32, trainable=False, name='k0_ramp')
    k0_ramp_new_ph = tf.compat.v1.placeholder(dtype=tf.float32, shape=[])
    k0_ramp_assign_op = tf.compat.v1.assign(k0_ramp_var, k0_ramp_new_ph)
else:
    k0_ramp_var = tf.constant(1., dtype=tf.float32)
    k0_ramp_new_ph = None
    k0_ramp_assign_op = None

def k0_step_hook(it):
    if not args.K0Loss:
        return
    frac = min(1.0, it / max(1, args.K0RampIters))
    sess.run(k0_ramp_assign_op, feed_dict={k0_ramp_new_ph: frac})
    if it % 100 == 0:
        print('K0 ramp @ it %d : %.4f' % (it, frac))

def _recirculation_length_from_line(u_line, x_line):
    neg = np.where(u_line < 0.)[0]
    if len(neg) == 0:
        return 0.0
    last_neg = neg[-1]
    if last_neg == len(u_line) - 1:
        return (x_line[-1] - (x_c + r_c)) / d
    pos_after = np.where(u_line[last_neg + 1:] >= 0.)[0]
    if len(pos_after) == 0:
        return (x_line[-1] - (x_c + r_c)) / d
    x_cross = x_line[last_neg + 1 + pos_after[0]]
    return (x_cross - (x_c + r_c)) / d

def diagnostics_step_hook(it):
    if not args.K0Loss or it % 2000 != 0:
        return
    if 'k0_wall_mean_rms' not in globals() or 'recirculation_centerline_u' not in globals():
        return
    wall_rms = sess.run(k0_wall_mean_rms)
    u_line = sess.run(recirculation_centerline_u)
    recirc_len = _recirculation_length_from_line(u_line, recirculation_x_line)
    print('K0 monitor @ it %d : wall mean tap RMS = %.3e, recirc length = %.3f D' % (it, wall_rms, recirc_len))
    if recirc_len > 2.0:
        raise RuntimeError('Recirculation length exceeded 2D abort threshold: %.3f D' % (recirc_len))

def training_step_hook(it):
    causal_step_hook(it)
    k0_step_hook(it)
    diagnostics_step_hook(it)
...
if args.K0WakeOnly:
    wake_mask = tf.cast(tf.logical_and(tf.logical_and(x_tf_int >= 1.0, x_tf_int <= 7.0), tf.abs(y_tf_int) <= 2.0), tf.float32)
    Loss_k0_wrap = tf.reduce_sum(wake_mask * k0_residual(x_tf_int, y_tf_int)) / (tf.reduce_sum(wake_mask) + 1e-8)
else:
...
if args.K0Loss:
    Loss = Loss + args.LambdaK0 * k0_ramp_var * Loss_k0_wrap
...
if args.SparseData and args.K0Loss and args.PressureOnly:
    n_time_cyl = max(1, len(pmes_cyl) // max(1, args.NTaps))
    x_k0_monitor = np.asarray(xmes_cyl[:args.NTaps], dtype=np.float32).reshape(-1, 1)
    y_k0_monitor = np.asarray(ymes_cyl[:args.NTaps], dtype=np.float32).reshape(-1, 1)
    p_k0_mean_target = np.reshape(np.asarray(pmes_cyl, dtype=np.float32), (n_time_cyl, args.NTaps)).mean(axis=0).reshape(-1, 1)
    x_tf_k0_monitor = tf.constant(x_k0_monitor, dtype=tf.float32)
    y_tf_k0_monitor = tf.constant(y_k0_monitor, dtype=tf.float32)
    p_tf_k0_monitor = tf.constant(p_k0_mean_target, dtype=tf.float32)
    k0_wall_mean_rms = tf.sqrt(tf.reduce_mean(tf.square(tf.real(fluid_p(x_tf_k0_monitor, y_tf_k0_monitor)[0,:,0]) - tf.reshape(p_tf_k0_monitor, [-1]))))
    recirculation_x_line = np.linspace(x_c + r_c + 1e-3, x_c + 6.*d, 200, dtype=np.float32)
    recirculation_centerline_u = tf.real(fluid_u(tf.constant(recirculation_x_line.reshape(-1, 1), dtype=tf.float32), tf.constant(np.zeros((200, 1), dtype=np.float32), dtype=tf.float32))[0,:,0])
    print('K0 monitor ready: %d taps x %d times' % (args.NTaps, n_time_cyl))
...
List_it_loss_LBFGS,List_it_loss_valid_LBFGS = nnf.model_train_scipy(opt_LBFGS,sess,Loss,tf_dict[0],List_loss = True,tf_dict_valid=tf_dict_valid,loss_valid = Loss_dense_mes,step_hook=training_step_hook)
...
AdamTmax = Tmax-(t2-t0)
List_it_loss_Adam,List_it_loss_valid_Adam = nnf.model_train_Adam(opt_Adam,sess,Loss,liste_tf_dict=tf_dict,Nit=1e5,tolAdam=1e-5,it=it,itdisp=100,maxTime=AdamTmax,multigrid=multigrid,NgridTurn=NgridTurn,List_loss = True,tf_dict_valid=tf_dict_valid,loss_valid = Loss_dense_mes,step_hook=training_step_hook)
t3 = time.time()
...
if args.K0Loss:
    print('K0 ramp final value : %.4f' % (sess.run(k0_ramp_var)))
